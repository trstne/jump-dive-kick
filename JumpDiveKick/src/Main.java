...

    {
        ...
        new AnimationTimer() //Begins an animation timer, which allows for actions to be reflected within a "game time"
        {
            public void handle(long currentNanoTime)  { //code within the handle called for every frame that the Animation Timer is active
                ...
                if (input.contains("A"))
                {
                    if (PlayerA.positionY == 300) //If the player is not jumping, and on the ground
                    {
                ...
                {
                    if (PlayerB.positionY < 300  &&  !PlayerB.jumpBackAnimation && !input.contains("UP"))
                    {
                ...
                if (input.contains("UP") && PlayerB.positionY >= 250 && !input.contains("LEFT")){
                    PlayerB.jump = true;
                }
